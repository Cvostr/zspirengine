...
typedef struct ZSVERTEX {
    ...
    ZSVERTEX(Vec3 npos, Vec2 nuv) : pos(npos.X, npos.Y, npos.Z), uv(nuv.X, nuv.Y) , normal(0) {
        bones_num = 0;
    }
    ZSVERTEX(Vec3 npos, Vec2 nuv, Vec3 nnormal) : pos(npos.X, npos.Y, npos.Z), uv(nuv.X, nuv.Y), normal(nnormal.X, nnormal.Y, nnormal.Z) {
        bones_num = 0;
        tangent = Vec3(0);
        bitangent = Vec3(0);
    }
